...
public class Frost extends Script implements Paintable, MessageListener{
	...
	public static int foodId = 386;    //sharks
   
	public static int numAtkPots = 1;      
	public static int numStrPots = 1;  
	public static int numFood;      // number of food to withdraw
	public static int numPrayer;    // number of prayer renewals to withdraw
	public static int hp;           // hp to eat at
	...
	public static boolean drinkRenewal = true;
	...
	public static Area frostlair = new Area(new Tile(2819, 3745), new Tile(2819, 3767), new Tile(2870, 3767), new Tile(2871, 3745));
    ...
    public boolean onExecute() {
		  ...
		  //strategies.add(new QuickPrayer());
	...
	public void messageReceived(MessageEvent m) {
		if((m.getMessage().contains("These entities are already under attack.")) && m.getType() == 0){
			taken = true;
			Time.sleep(50, 100);
		}	
		if ((m.getMessage().contains("@369@Your prayer renewal has run out.") && m.getType() == 0)){
			drinkRenewal = true;
		}
		if ((m.getMessage().contains("You have run out of prayer points!") || (m.getMessage().contains("You do not have enough prayer points."))) && m.getType() == 0){
			quickPrayerStatus = false;
		}
	}
